...
def load_referential_data(args, e3cover_csv, scans_split):
    """
    :param args:
    :param e3cover_csv:
    :param scans_split:
    :return:
    """
    e3cover_data = pd.read_csv(e3cover_csv)

    if args.mentions_target_class_only:
        n_original = len(e3cover_data)
        e3cover_data = e3cover_data[e3cover_data['mentions_target_class']]
        e3cover_data.reset_index(drop=True, inplace=True)
        print('Dropping utterances without explicit '
              'mention to the target class {}->{}'.format(n_original, len(e3cover_data)))

    e3cover_data = e3cover_data[['tokens', 'instance_type', 'scan_id',
                                 'dataset', 'target_id', 'utterance', 'stimulus_id']]
    e3cover_data.tokens = e3cover_data['tokens'].apply(literal_eval)

    # Add the is_train data to the pandas data frame (needed in creating data loaders for the train and test)
    is_train = e3cover_data.scan_id.apply(lambda x: x in scans_split['train'])
    e3cover_data['is_train'] = is_train

    # Trim data based on token length
    train_token_lens = e3cover_data.tokens[is_train].apply(lambda x: len(x))
    print('{}-th percentile of token length for remaining (training) data'
          ' is: {:.1f}'.format(95, np.percentile(train_token_lens, 95)))
    n_original = len(e3cover_data)
    e3cover_data = e3cover_data[e3cover_data.tokens.apply(lambda x: len(x) <= args.max_seq_len)]
    e3cover_data.reset_index(drop=True, inplace=True)
    print('Dropping utterances with more than {} tokens, {}->{}'.format(args.max_seq_len, n_original, len(e3cover_data)))
    ...
    if args.augment_with_sr3d is not None:
        ...
        sr3d = sr3d[is_train]
        sr3d = sr3d[e3cover_data.columns]
        print('Dataset-size before augmentation:', len(e3cover_data))
        e3cover_data = pd.concat([e3cover_data, sr3d], axis=0)
        e3cover_data.reset_index(inplace=True, drop=True)
        print('Dataset-size after augmentation:', len(e3cover_data))

    context_size = e3cover_data[~e3cover_data.is_train].stimulus_id.apply(lambda x: decode_stimulus_string(x)[2])
...
def compute_auxiliary_data(e3cover_data, all_scans, args):
    """Given a train-split compute useful quantities like mean-rgb, a word-vocabulary.
    :param e3cover_data: pandas Dataframe, as returned from load_referential_data()
    :param all_scans:
    :param args:
    :return:
    """
    ...
    if args.vocab_file:
        ...
        train_tokens = e3cover_data[e3cover_data.is_train]['tokens']
    ...
    training_scan_ids = set(e3cover_data[e3cover_data['is_train']]['scan_id'])
    ...
    prc = 99
    testing_scan_ids = set(e3cover_data[~e3cover_data['is_train']]['scan_id'])
...
def trim_scans_per_e3covernet_data(e3cover_data, scans):
    # remove scans not in e3cover_data
    in_r3d = e3cover_data.scan_id.unique()
    to_drop = []
    for k in scans:
        if k not in in_r3d:
            to_drop.append(k)
