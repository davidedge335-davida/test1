...
Output is a .csv that can be used to do training, analysis etc. of the E3CoverNet data.
"""
import argparse
import pprint
...
from e3covernet.in_out.nr3d import load_nr3d_raw_data
from e3covernet.in_out.sr3d import load_sr3d_raw_data
from e3covernet.in_out.neural_net_oriented import load_scan_related_data
from e3covernet.utils import read_lines

from e3covernet.data_generation.nr3d.tokenization import pre_process_text
from e3covernet.data_generation.nr3d.tokenization import sentence_spelling_dictionary
from e3covernet.data_generation.nr3d.tokenization import token_spelling_dictionary

from e3covernet.analysis.utterances import mentions_target_class


def parse_arguments():
    parser = argparse.ArgumentParser(description='Prepare Referntial Data For E3CoverNet')
    ...
    parser.add_argument('--nr3d-file', type=str,
                        help='raw Nr3D CSV (required when -type nr3d)')
    parser.add_argument('--sr3d-file', type=str,
                        help='raw Sr3D CSV (required when -type sr3d)')
    parser.add_argument('--word-freq-file', type=str,
                        help='SymSpell frequency dictionary; defaults to the bundled dictionary')
    parser.add_argument('--vocab-file', type=str,
                        help='newline-delimited vocabulary used to validate Nr3D tokens')

    args = parser.parse_args()

    if args.type == 'nr3d' and not args.nr3d_file:
        parser.error('--nr3d-file is required when -type nr3d')
    if args.type == 'sr3d' and not args.sr3d_file:
        parser.error('--sr3d-file is required when -type sr3d')
    if args.type == 'nr3d' and not args.vocab_file:
        parser.error('--vocab-file is required when -type nr3d')

    if args.word_freq_file is None:
        package_root = Path(__file__).resolve().parents[1]
        args.word_freq_file = str(
            package_root / 'data/language/symspell_frequency_dictionary_en_82_765.txt')

    # Print them nicely after computed defaults have been applied.
...
def main():
    ...
    if args.type == 'nr3d':
        # Read the vocabulary used in preprocessing and sanity checks.
